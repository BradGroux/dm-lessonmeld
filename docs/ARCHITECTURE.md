...
- The support target owns app-adjacent pure workflow logic such as project render planning, quick-recording capture settings and completion render/package decisions, caption/transcript sidecar export, and typed project-editor dirty-state transitions. Quick-recording manifests persist the actual per-recording capture toggles, including an in-session interaction-metadata opt-out, rather than falling back to saved defaults.
