...
- App termination is routed through an AppKit termination coordinator. Active recordings require explicit confirmation, termination waits for recording cleanup, and launch diagnostics are marked clean only after capture state is idle.
- The persisted main-window launch preference selects SwiftUI's presented or suppressed launch behavior. App termination and local-control services initialize independently of window presentation so menu-bar-only launches remain operational.
- Shortcut settings report the actual built-in native bindings as read-only status. Reserved annotation color shortcuts are labeled inactive, and command-palette metadata uses the same presentation policy instead of implying that persisted custom text changes runtime handlers.
- Settings remain staged until Save All, and closing the Settings window guards unsaved edits with explicit save, discard, and cancel choices. Backup controls use the visible staged settings for enablement, root selection, JSON output, planning, and commits so their actions match the values on screen.
- The support target owns app-adjacent pure workflow logic such as project render planning, quick-recording capture settings and duration policy, completion render/package decisions, caption/transcript sidecar export, and typed project-editor dirty-state transitions. Quick-recording manifests persist the actual per-recording capture toggles, including an in-session interaction-metadata opt-out, rather than falling back to saved defaults. Manual-stop recordings use the shared maximum validated capture duration and still end immediately when the user presses Stop.
...
- Editor dirty tracking compares typed `Equatable` snapshots for metadata, markers, edit decisions, editor settings, overlays, and captions. The snapshots include every value written by the editor settings save path, including automatic click zooms, so user text cannot collide through serialization delimiters and newly tracked fields are covered by compile-time structure and focused tests.
- Decoded editor settings are reconstructed through their clamping initializers before use. Render planning rejects custom canvas dimensions whose requested preset would produce non-finite, odd, undersized, or larger-than-7680-pixel output, and the renderer repeats the final dimension guard after resolving real source media geometry.
...
- Video import builds new bundles in a sibling staging directory before promotion and stages existing-project media under a unique project-local name before committing the manifest. Failed imports remove only artifacts created by that attempt and report incomplete cleanup without exposing local paths.
- CLI project recording validates the complete typed request before writing output, runs requested screen, microphone, and webcam captures as one structured operation, and cancels and settles sibling captures when any capture fails. It writes into a hidden sibling staging bundle, removes that staging bundle on failure or cancellation, and promotes it to the requested `.dmlm` path only after the media and manifest are complete.
- Project-scoped asynchronous app work uses an explicit generation token. Opening, creating, importing, or closing a project invalidates the prior generation, cancels tracked render/export/package/import/preview tasks, records active jobs as cancelled, and prevents delayed completions from mutating the next project's state.
- LearnHouse and connector package builders safely recreate their destination package directory before every export so stale files cannot survive into regenerated manifests, checksums, or archives.
