...
- The app editor opens local `.dmlm` bundles, inspects manifests, opens video-backed projects in a preview-first timeline workspace, saves `editor-settings.json` canvas settings, `edit-decision-list.json` cut/zoom sidecars, and `overlays.json` timed overlay sidecars, exports those saved cuts, checks render readiness, exports full renders with canvas styling, cursor/click/shortcut/zoom/overlay/annotation/caption burn-in and progress/cancel controls, and creates contiguous trim exports through existing core services.
